...
public class MyPortfolioPage {
	...
	public void verifyPortfolio(String portfolioName) {
		try {
			...
			Base.test.log(Status.FAIL, "Unable to verify if Portfolio name is non existing..." + e.getMessage());
			Assert.fail("Unable to verify if Portfolio name is non existing..." + e.getMessage());
		}
	}
